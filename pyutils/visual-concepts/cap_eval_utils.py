...
def calc_pr_det(counts, out, thresh):
  """
  [P, R, score, ap] = calc_pr_det(counts, out)
  Input    :
    counts : occurrence of this word in the ith image
    out    : score for this image
    thresh : threshold for scoring this image
  Output   :
    P, R   : precision and recall
    score  : score which corresponds to the particular precision and recall
    ap     : average precision
  """
  n_image = out.shape[0]
  thr_out = np.array([1 if out[i] >= thresh else 0 for i in range(n_image)])
  tog = np.hstack((counts[:,np.newaxis].astype(np.float64),
                   thr_out[:, np.newaxis].astype(np.float64)))
  ind = np.argsort(out)
  ind = ind[::-1]
  score = np.array([tog[i,1] for i in ind])
  sortcounts = np.array([tog[i,0] for i in ind])

  tp, fp = np.zeros_like(score), np.zeros_like(score)
  for i in xrange(sortcounts.shape[0]):
    if sortcounts[i] == 0 and score[i] == 1:
      fp[i] = 1.;
    elif sortcounts[i] == 1 and score[i] == 1:
      tp[i] = 1.;

  P = np.cumsum(tp)/(np.cumsum(tp) + np.cumsum(fp));

  # c = accumarray(sortcounts(:)+1, 1);
  numinst = np.sum(sortcounts, axis=0)
  R = np.cumsum(tp)/numinst
