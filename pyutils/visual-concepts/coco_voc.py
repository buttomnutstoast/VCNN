...
class coco_voc():
  def __init__(self, image_set, devkit_path=None,  image_path=None):
    ...
    assert(image_set == 'train' or image_set == 'val' or image_set == 'test')
    ...
    caption_prefix = 'train' if image_set == 'train' else 'val'
    self._coco_caption_data = COCO(
      os.path.join(self._devkit_path,
                   'annotations',
                   'captions_' + caption_prefix + '2014.json')
      );

    self._coco_instances_data = COCO(
      os.path.join(self._devkit_path,
                   'annotations',
                   'instances_' + caption_prefix + '2014.json')
      );
  ...
  def _load_image_set_index(self):
    ...
    image_set_file = os.path.join('dataset/mscoco_vc/splits', self._image_set + '.ids')
  ...
  @property
  def coco_instances_data(self):
      return self._coco_instances_data
