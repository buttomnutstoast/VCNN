...
def get_vocab_counts_det(image_ids, coco_insts, map_file, vocab):
  det2cap = sg_utils.det2cap(map_file, vocab)
  det_cats = det2cap.keys()
  cats2ind = {det_cats[i]: i for i in range(len(det_cats))}

  id2cats = coco_insts.cats

  counts = np.zeros((len(image_ids), len(det_cats)), dtype = np.float)
  for i in xrange(len(image_ids)):
    ann_ids = coco_insts.getAnnIds(image_ids[i])
    anns = coco_insts.loadAnns(ann_ids)
    for ann in anns:
      ann_id = ann['category_id']
      ann_name = id2cats[ann_id]['name']
      if ann_name in cats2ind.keys():
        ann_ind = cats2ind[ann_name]
        counts[i, ann_ind] = 1
  return counts
